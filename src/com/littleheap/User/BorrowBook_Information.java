...
public class BorrowBook_Information extends JPanel implements ActionListener {
	...
	public static void setTextArea() {
		...
		for(int i=0;i<Information.bookarray.size();i++) {
            ...
            info = info+(Information.bookarray.get(i).current == null ? "" : Information.bookarray.get(i).current);
			int lim = Information.bookarray.get(i).current == null ? 0 : Information.bookarray.get(i).current.length();
            for(int j=0;j<(6-lim);j++) {
            	info = info +" ";
            }
			String tmp_dateoff = Information.bookarray.get(i).dateoff == null ? "" : Information.bookarray.get(i).dateoff;
            info = info+"     "+tmp_dateoff+"\n";
